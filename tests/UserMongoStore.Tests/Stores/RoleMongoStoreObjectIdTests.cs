...
public sealed class RoleMongoStoreObjectIdTests
{
    ...
    public void Setup()
    {
        ...
        var database = client.GetDatabase("RoleMongoStoreObjectIdTests");
        
        var roleCollection = database.GetCollection<ApplicationRoleObjectId>("ApplicationRole");

        _roleMongoStore =
            new RoleMongoStoreAsObjectId<ApplicationRoleObjectId>(new IdentityErrorDescriber(), roleCollection);
    }
    ...
    {
        var role = new ApplicationRoleObjectId()
        {
            Name = "role_test",
