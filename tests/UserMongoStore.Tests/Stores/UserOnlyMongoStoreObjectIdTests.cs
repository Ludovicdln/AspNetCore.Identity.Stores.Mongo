...
public sealed class UserOnlyMongoStoreObjectIdTests
{
    private UserOnlyMongoStoreAsObjectId<ApplicationUserObjectId> _userMongoStore;
    ...
    public void Setup()
    {
        ...
        var database = client.GetDatabase("UserOnlyMongoStoreObjectIdTests");

        var userCollection = database.GetCollection<ApplicationUserObjectId>("ApplicationUser");

        _userMongoStore =
            new UserOnlyMongoStoreAsObjectId<ApplicationUserObjectId>(new IdentityErrorDescriber(), userCollection);
    }
