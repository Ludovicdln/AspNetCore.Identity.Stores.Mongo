...
public sealed class UserMongoStoreObjectIdTests
{
    private UserMongoStoreAsObjectId<ApplicationUserObjectId, ApplicationRoleObjectId> _userMongoStore;
    private RoleMongoStoreAsObjectId<ApplicationRoleObjectId> _roleMongoStore;
    ...
    public void Setup()
    {
        ...
        var database = client.GetDatabase("UserMongoStoreObjectIdTests");

        var userCollection = database.GetCollection<ApplicationUserObjectId>("ApplicationUser");

        var roleCollection = database.GetCollection<ApplicationRoleObjectId>("ApplicationRole");

        _userMongoStore =
            new UserMongoStoreAsObjectId<ApplicationUserObjectId, ApplicationRoleObjectId>(new IdentityErrorDescriber(), userCollection,
                roleCollection);

        _roleMongoStore = new RoleMongoStoreAsObjectId<ApplicationRoleObjectId>(new IdentityErrorDescriber(), roleCollection);
    }
    ...
    {
        var user = new ApplicationUserObjectId()
        {
    ...
    public async Task FindByLogin_ShouldReturnSuccess()
    {
        ...
        var userLoginInfo2 = new MongoUserLoginInfo("login_provider_test_2", "key_provider_test_2", "login_name_test_2");
        ...
        var userExist2 = await _userMongoStore.FindByLoginAsync(userLoginInfo2.LoginProvider, userLoginInfo2.ProviderKey);
    ...
    [Test]
    public void  CreateIdentityUserAsObjectId_ShouldGenerateObjectId()
    {
        var user = new ApplicationUserObjectId()
        {
            Email = "test2@gmail.com",
        ...
        Assert.That(user.Id, Is.Not.EqualTo(ObjectId.Empty));
    }
    ...
    public async Task UpdateAllProperties_ShouldReturnSuccess()
    {
        ...
        user.Roles = new List<MongoRole<ObjectId>>()
            { new(ObjectId.GenerateNewId(), "role1_test", "ROLE1_TEST"), new(ObjectId.GenerateNewId(), "role2_test", "ROLE2_TEST") };
    ...
    public async Task SetToken_ShouldCreateToken()
    {
        ...
        await _userMongoStore.SetTokenAsync(user, tokenExpected.LoginProvider, tokenExpected.Name, tokenExpected.Value , new CancellationToken());
        
        var userExist = await _userMongoStore.FindByIdAsync(user.Id.ToString());

        var stockedToken = await _userMongoStore.GetTokenAsync(userExist, tokenExpected.LoginProvider, tokenExpected.Name, new CancellationToken());
    ...
    public async Task SetToken_ShouldReplaceValue()
    {
        ...
        await _userMongoStore.SetTokenAsync(user, tokenExpected.LoginProvider, tokenExpected.Name, "newValueOfToken" , new CancellationToken());
    ...
    public async Task RemoveToken_ShouldSuccess()
    {
        ...
        await _userMongoStore.RemoveTokenAsync(user, tokenExpected.LoginProvider, tokenExpected.Name, new CancellationToken());
    ...
    public async Task AddLoginsAlreadyExist_ShouldNotSuccess()
    {
        ...
        user.Logins.Add(new MongoUserLoginInfo(userLoginInfo.LoginProvider, userLoginInfo.ProviderKey, userLoginInfo.ProviderDisplayName));
    ...
    public async Task GetUsersForClaims_ShouldReturnUsers()
    {
        ...
        var user2 = new ApplicationUserObjectId()
        {
    ...
    [Test]
    public async Task AddToRole_ShouldSuccess()
    {
        var user = new ApplicationUserObjectId()
        {
            Email = "test_roles_2@gmail.com",
            NormalizedEmail = "test_roles_2@gmail.com".ToUpper(),
            PasswordHash = "12345678",
            UserName = "test_roles_2",
            NormalizedUserName = "test_roles_2".ToUpper()
        };

        await _userMongoStore.CreateAsync(user);
        
        var appRole = new ApplicationRoleObjectId()
        {
            Name = "role_test_3",
            NormalizedName = "role_test_3".ToUpper()
        };
        
        await _roleMongoStore.CreateAsync(appRole, new CancellationToken());

        await _userMongoStore.AddToRoleAsync(user, appRole.NormalizedName);

        var userExist = await _userMongoStore.FindByIdAsync(user.Id.ToString());

        var firstRole = userExist.Roles.FirstOrDefault();
        
        Assert.Multiple(() =>
        {
            Assert.That(userExist.Roles, Has.Count.EqualTo(1));
            Assert.That(firstRole, Is.Not.Null);
            Assert.That(firstRole.NormalizedName, Is.EqualTo(appRole.NormalizedName));
            Assert.That(firstRole.Name, Is.EqualTo(appRole.Name));
        });
    }
    
    [Test]
    public async Task AddToRoleAlreadyExist_ShouldNotSuccess()
    {
        var user = new ApplicationUserObjectId()
        {
            Email = "test_roles_2@gmail.com",
        ...
        user.Roles.Add(new MongoRole<ObjectId>(appRole.Id, appRole.Name, appRole.NormalizedName));
    ...
    [Test]
    public async Task IsInRole_ShouldReturnFalse()
    {
        var user = new ApplicationUserObjectId()
        {
            Email = "test_roles@gmail.com",
            NormalizedEmail = "test_roles@gmail.com".ToUpper(),
            PasswordHash = "12345678",
            UserName = "test_roles",
            NormalizedUserName = "test_roles".ToUpper()
        };

        await _userMongoStore.CreateAsync(user);

        var result = await _userMongoStore.IsInRoleAsync(user, "ROLE_TEST");
        
        Assert.That(result, Is.False);
    }
    
    [Test]
    public async Task IsInRole_ShouldReturnTrue()
    {
        var user = new ApplicationUserObjectId()
        {
            Email = "test_roles@gmail.com",
        ...
        var role = new MongoRole<ObjectId>(appRole.Id, "role_test", "role_test".ToUpper());
    ...
    [Test]
    public async Task GetRoles_ShouldReturnListOfNames()
    {
        var user = new ApplicationUserObjectId()
        {
            Email = "test_roles@gmail.com",
        ...
        var appRole2 = new ApplicationRoleObjectId()
        {
            Name = "role_test_5",
            NormalizedName = "role_test_5".ToUpper()
        };
        
        var role = new MongoRole<ObjectId>(appRole.Id, appRole.Name, appRole.NormalizedName.ToUpper());
        
        var role2 = new MongoRole<ObjectId>(appRole2.Id, appRole2.Name, appRole2.NormalizedName.ToUpper());
        
        await _roleMongoStore.CreateAsync(appRole, new CancellationToken());
        
        await _roleMongoStore.CreateAsync(appRole2, new CancellationToken());
        ...
        var rolesNames = await _userMongoStore.GetRolesAsync(user);
        
        Assert.Multiple(() =>
        {
            Assert.That(rolesNames[0], Is.EqualTo(role.Name));
            Assert.That(rolesNames[1], Is.EqualTo(role2.Name));
            Assert.That(rolesNames, Has.Count.EqualTo(2));
        });
    }
    
    [Test]
    public async Task RemoveFromRole_ShouldSuccess()
    {
        var user = new ApplicationUserObjectId()
        {
            Email = "test_roles@gmail.com",
        ...
        await _userMongoStore.RemoveFromRoleAsync(user, role.NormalizedName!);
        ...
        {
            Assert.That(userExist.Roles, Has.Count.EqualTo(1));
            Assert.That(userExist.Roles[0].Name, Is.EqualTo(role2.Name));
            Assert.That(userExist.Roles[0].NormalizedName, Is.EqualTo(role2.NormalizedName));
        });
    }
    
    [Test]
    public async Task GetUsersInRole_ShouldReturnListOfUser()
    {
        var user = new ApplicationUserObjectId()
        {
            Email = "test_roles@gmail.com",
        ...
        {
            Name = "role_test_10",
            NormalizedName = "role_test_10".ToUpper()
        };
        ...
        user2.Roles.Add(new MongoRole<ObjectId>(appRole.Id, appRole.Name, appRole.NormalizedName));

        await _userMongoStore.CreateAsync(user);

        await _userMongoStore.CreateAsync(user2);

        var users = await _userMongoStore.GetUsersInRoleAsync(appRole.NormalizedName);
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(users[1].Id, Is.EqualTo(user2.Id));
        });
    }
}